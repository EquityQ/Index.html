...
local sv_maxtick_slider = ui.new_slider("AA","Fake lag", "[QingYiMingYue] Maximum Tick Setting", 4, 62, 16)

client.set_event_callback("setup_command", function()
    ui.set(sv_maxusrcmdprocessticks,ui.get(sv_maxtick_slider))
end)

local fakelag_cob_1 = ui.new_combobox("AA","Fake lag", "[QingYiMingYue] Fake Lag Mode Setting Mode","Dynamic","Maximum","Fluctuate","Dynamic/Maximum")
...
local onfire_waiting_value =ui.new_slider("AA","Fake lag", "[QingYiMingYue] On Fire Fake lag Limit Waiting Time", 1, 2000, 300,true,"ms")

local fakelag_cob = ui.new_combobox("AA","Fake lag", "[QingYiMingYue] Fake Lag Limit Setting Random Mode","Fixed","Random","On Shoot","Static")
local fixed_value =ui.new_slider("AA","Fake lag", "[QingYiMingYue] Fixed Fake lag Limit Value", 3, 60, 14)
local fixed_minimum =ui.new_slider("AA","Fake lag", "[QingYiMingYue] Fixed Fake lag Limit Minimum Value", 3, 60, 14)
local fixed_maximum =ui.new_slider("AA","Fake lag", "[QingYiMingYue] Fixed Fake lag Limit Maximum Value", 3, 60, 14)

local random_minimum =ui.new_slider("AA","Fake lag", "[QingYiMingYue] Random Fake lag Limit Minimum Value", 3, 60, 14)
local random_maximum =ui.new_slider("AA","Fake lag", "[QingYiMingYue] Random Fake lag Limit Maximum Value", 3, 60, 14)

local static_value =ui.new_slider("AA","Fake lag", "[QingYiMingYue] Static Fake lag Limit Value", 3, 60, 14)

local on_shoot_value = ui.new_slider("AA","Fake lag", "[QingYiMingYue] On Shoot Fake lag Limit Value", 1, 60, 6)
...
local double_tap_chk = ui.new_checkbox("RAGE","Other", "[QingYiMingYue] Unsafe Double Tap")
function set_unsafe()
